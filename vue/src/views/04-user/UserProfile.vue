<template>
    ...
    <UserCard :user-profile="userProfile" :user-id="userId" :user-age="userAge" :star-sign="starSign"
        ...
        @follow="follow" @unfollow="unfollow" @update="update" @delete="dodelete" @ban="ban" @unban="unban" @report="report"
        @showBanList="showBanList" />

    <BanList v-if="showBanListModal" :show="showBanListModal" :banned-users="bannedUsers" @close="closeBanList" @unban="unbanUser" />
...
onMounted(function () {
    ...
    findById();
     if (userStore && userStore.id) {
        console.log("第一次請求追蹤狀態");
        ifFollow();
        checkBanStatus();
        checkReportStatus();
        console.log("ifFollowing"+ifFollowing.value)
        console.log(ifFollowing.value)
...
function follow() {
    ...
    axiosapi.post('/secure/users/follow/', request.data, { headers: request.headers }
    ).then(function (response) {
        console.log("追蹤成功!");
        ifFollowing.value = response.data.ifFollowing;
        userProfile.data.followerNum ++;
        matching(true);
    }).catch(function (error) {
        console.log("追蹤失敗!" + error);
    });
}
...
function unfollow() {
    ...
    axiosapi.delete('/secure/users/follow/', request
    ).then(function (response) {
        console.log("取消追蹤成功!");
        ifFollowing.value = response.data.ifFollowing;
        userProfile.data.followerNum --;
...
function unban() {
    Swal.fire({
        ...
        if(result.isConfirmed){
            ...
            axiosapi.delete('/users/ban/', request)
                .then(function(response){
                    console.log("成功取消封鎖");
                    bannedUsers.value = bannedUsers.value.filter(user => user.id !== banedUserId)
                    ifBan.value = response.data.ifBan;
...
function showBanList() {
    ...
    axiosapi.get('/users/banList', request)
    .then(function(response) {
        bannedUsers.value = response.data.map(ban => ({
        id: ban.id.banedUserId,
        name: ban.banedUserId.userName
        }));
